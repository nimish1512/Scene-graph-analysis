...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        if cfg.MODEL.ALGORITHM == "sg_baseline":
            ...
            #
            #
            # Adding transformer to the group of networks
            #
            #
            self.transformer = build_transformer_model(cfg)
            self.temporal_map = torch.ones([1,512],dtype=torch.float64)
    ...
    def forward(self, features, proposals, targets=None):
        ...
        if self.training:
            ...
            if self.cfg.MODEL.USE_RELPN:
                #
                #
                # Combining temporal features and dense graph
                #
                #
                proposals = torch.mm(proposals,self.temporal_map)
        ...
        if self.cfg.MODEL.USE_FREQ_PRIOR:
            ...
            self.temporal_map = self.transformer((x))
