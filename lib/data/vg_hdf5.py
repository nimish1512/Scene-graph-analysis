...
def load_graphs(graphs_file, images_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    print("Graph_file: {}".format(graphs_file))
    im_h5 = h5py.File(images_file, 'r')
    print("Images_File: {}".format(images_file))
    ...
    for i in range(len(image_index)):
        ...
        if im_to_first_rel[i] >= 0:
            ...
            # Image 1 has 4 objects
            #   obj_1 which is the obj_index here has N predicates attached to it

            # rels would be an array that has ((object id, numpy array of all possible relations from that object in that image))
            # ((1,['behind','in front of','besides']))
        else:
